...
/**
 * 评论观点抽取 或 情感识别 [可能他们的训练标本不够吧] 垃圾 很容易没结果
 */
 ...
 * 中文DNN语言模型
 */
func (this *AiText) DnnLm(intxt string) map[string]interface{} {
	post_arg := map[string]interface{}{
		"input_sequence": ghostlib.UrlEncode(ghostlib.ConvertStrEncode(intxt, "utf-8", "gbk")),
	}
	post_json, err := json.Marshal(post_arg)
	if nil != err {
		panic(err)
	...
	real_uri := this.getInterFaceUri(DNNL_API_URI)
	req, err := http.NewRequest("POST", real_uri, bytes.NewReader(post_json))
	resp, _ := this.client.Do(req)
	...
	return map_result["result"].(map[string]interface{})
}

/**
 * 短文本相似度
 */
func (this *AiText) SimNet(ins1, ins2 string) map[string]interface{} {
	post_arg := map[string]interface{}{
		"input": map[string]interface{}{
			"qslots": []interface{}{
				map[string]interface{}{
					"terms_sequence": ghostlib.UrlEncode(ghostlib.ConvertStrEncode(ins1, "utf-8", "gbk")),
					"type": 0,
					"items": []interface{}{},
				},
			},
			"tslots": []interface{}{
				map[string]interface{}{
					"terms_sequence": ghostlib.UrlEncode(ghostlib.ConvertStrEncode(ins2, "utf-8", "gbk")),
					"type": 0,
					"items": []interface{}{},
				},
			},
	...
	real_uri := this.getInterFaceUri(SIMNET_API_URI)
	req, err := http.NewRequest("POST", real_uri, bytes.NewReader(post_json))
	resp, _ := this.client.Do(req)
	...
	return map_result["output"].(map[string]interface{})
}
